...
router.post("/edit_data", upload.single("icon"), function (req, res, next) {
  console.log(req.file);
  pool.query(
    "update vehicle set categoryid=?,subcategoryid=?,companyid=?,modelid=?,vendorid=?,registrationno=?,color=?,fueltype=?,ratings=?,average=?,remarks=?,capacity=?,status=?,feature=? where vehicleid=?",
    [
      req.body.categoryid,
      req.body.subcategoryid,
      req.body.companyid,
